...
make_pairtree_input <- function(rdata_input, cn_master, cn_pattern, pp_pattern, which_genome, tag){
  ...
  psm <- tibble::tibble(id = mut_df_filt$id,
                        ...
                        Consequence = as.character(mut_df_filt$Consequence),
                        IMPACT = as.character(mut_df_filt$IMPACT),
                        SYMBOL = as.character(mut_df_filt$SYMBOL),
                        HGVSc = as.character(mut_df_filt$HGVSc))
